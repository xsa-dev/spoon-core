...
## Prerequisites
- Python 3.12 (recommended inside a virtual environment).
...
## Notes and tips
- Broadcasting is off by default to avoid spending funds; enable explicitly and ensure accounts are funded.
- If you see `ModuleNotFoundError`, install the extra deps noted above and rerun from the repo root.
- Policy errors usually mean the signing identity is not authorized for the chain/amount/target; check your Turnkey org policy and the chain ID of the unsigned transaction.
